...
import type { AppState, Story, VocabItem, Progress } from './types';
    ...
    loadProgress: () => {
      if (typeof window !== 'undefined') {
        ...
        if (saved) {
          ...
          if (progress.lastActiveDate && progress.lastActiveDate !== today && !isYesterday(progress.lastActiveDate)) {
            progress.streak = 0;
          }
    ...
    completeStory: () => {
      update(state => {
        ...
        if (state.progress.lastActiveDate !== today) {
          if (!state.progress.lastActiveDate || isYesterday(state.progress.lastActiveDate) || state.progress.lastActiveDate === today) {
